...
contract digiLocker {
    ...
    function getDocIndex(bytes32 docid,address uaddr)public view returns(uint256){
       
       for(uint i=0;i < ownerDocuments[uaddr].length ;i++){
           if(ownerDocuments[uaddr][i].docid == docid){
               return i;
           }
       }
       revert("Document not found");
    ...
    function getPublicKey(address uaddr)public view returns(string memory){
        return registerUsers[uaddr].pubKey;
    }
